...
test("GM disease inventory tracks all fourteen reviewed source diseases", () => {
  ...
  assert.equal(inventory.entries.filter((entry) => entry.status === "full").length, 6);
  assert.equal(inventory.entries.filter((entry) => entry.status === "partial").length, 8);
});
...
test("all six publishable GM diseases pass the 0.1.50 content contract", () => {
  assert.equal(diseases.length, 6);
...
test("Brain Worms remains partial after 0.1.50 because confusion attack rewriting and confused-ending exception are still unsupported", () => {
...
test("runtime seed contains exactly the six publishable GM diseases and no CLI-only _key", () => {
